...
TEXT
  ...
  def to_hash
    ...
    params.merge!(self.parameters.symbolize_keys) if self.parameters
    params
  end
  ...
  def add_note(pitch, start_step, length)
    ...
    self.note_on_pattern.pattern_indexes += [start_step]
    self.note_off_pattern.pattern_indexes += [end_step]
    save
  end
  ...
  def remove_note(start_step)
    if active_at_step(start_step)
      ...
      if pitch
        end_step = note_off_pattern.pattern_indexes.select { |x| x >= start_step }.sort[0]
        clear_range(start_step, end_step)
        save!
        true
      else
  ...
  def clear_range(start_step, end_step, skip_note_off = false)
    ...
    if self.active_at_step(end_step + 1)
      note_on_pattern.pattern_indexes += [end_step + 1]
      pitches[end_step + 1] = pitch_at_step(end_step + 1)
    end
    # clear contents
    (start_step..end_step).each { |index| pitches[index] = nil }
    note_on_pattern.pattern_indexes -= (start_step..end_step).to_a
    note_off_pattern.pattern_indexes -= (start_step..end_step).to_a unless skip_note_off
    # add note_off if previous step is active.
    if self.active_at_step(start_step - 1) and start_step != 0
      note_off_pattern.pattern_indexes += [start_step - 1]
    end
  end
